# [Josiah Bradshaw](index.md){: .site-title}

<div class="profile-links">

<a href="{{ '/assets/resume.pdf' | relative_url }}" download>
  Resume<br>
  <span class="download-text">Download</span>
</a>

<a href="https://github.com/MassSarcasm" target="_blank">
  GitHub<br>
  <span class="download-text">Link</span>
</a>

<a href="https://www.linkedin.com/in/josiahbradshaw/" target="_blank">
  LinkedIn<br>
  <span class="download-text">Link</span>
</a>

</div>

<div class="nav-links">

<a href="{{ '/' | relative_url }}">Home</a>
<a href="{{ '/aboutme' | relative_url }}" class="active-about">About Me</a>
<a href="{{ '/projects' | relative_url }}">Projects</a>
<a href="{{ '/blog' | relative_url }}">Blog</a>
<a href="{{ '/links' | relative_url }}">Links</a>

</div>

</div>

---

<div class="about-page">

<img src="{{ '/assets/images/josiah.jpeg' | relative_url }}" alt="Josiah Bradshaw" class="about-photo">

<h2>Hello, and Welcome!</h2>

<p>I’m Josiah Bradshaw, a Junior at the University of North Carolina at Charlotte (UNC Charlotte), currently pursuing my Bachelor of Science in Data Science.</p>

<p>As a returning student, I’m excited to continue advancing my education while gaining hands-on experience in data science and analytics. Throughout my studies, I have developed an interest in using data to explore problems, identify patterns, and communicate meaningful insights through analysis and visualization.</p>

<p>I have completed two research projects and am currently working on additional projects that allow me to apply the skills I’ve developed throughout my coursework. I also hold an Associate Degree in Communications from Guilford Technical Community College (GTCC), which has provided me with a strong foundation in communication, research, and presenting information effectively.</p>

<p>I have been passionate about computers and technology for much of my life, always exploring new technologies and expanding my technical knowledge. Alongside my academic background, I hold a WISE Device Repair Certificate and certifications through Apple’s Independent Repair Provider (IRP) Program. These experiences have given me hands-on knowledge in hardware repair, troubleshooting, and technical problem-solving.</p>

<h2>Beyond Data Science</h2>

<p>While much of my time is spent learning, working with technology, and developing my skills in Data Science, I also enjoy staying active and spending time with my family.</p>

<p>I’m the oldest of eight siblings, with six brothers and one sister, which has given me plenty of experience with responsibility, patience, and leadership from an early age.</p>

<p>I’m also happily married to my high school sweetheart! Outside of school and technology, I’m a power lifter and enjoy challenging myself in the gym. My current bench press personal best is 355 pounds, and I’m always working toward getting stronger.</p>

<p>Now, I’m eager to combine my technical background, communication skills, and education in Data Science as I pursue my first internship and take the next step toward a career in the field.</p>

<p>Thank you for visiting my portfolio!</p>

</div>

---

## <u><strong>Skills</strong></u>

- **Programming Languages:** Python, Java, SQL, HTML, Jupyter Notebooks
- **Familiar Operating Systems:** macOS, Windows, Linux
- **Certifications:** IRP Apple Repair Certification, WISE 1, WISE 2, Samsung Repair Certification
- **Technical Skills:** Microsoft Office Suite, Device Technician, Generative AI, Data Automation, Advanced Linux Distribution Knowledge, Cloudflare
